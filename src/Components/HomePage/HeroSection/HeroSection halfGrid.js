...
function HeroSection() {
  ...
  const imageStyles = img => ({
    background: `url(${img}) top center / cover no-repeat fixed`,
    // backgroundPosition: {
    //   // xxs: "right -100px",
    //   xxs: "right -160px",
    //   sm: "-150px -150px",
  ...
  return (
    <Paper
      ...
      sx={{
        ...
        // minHeight: `min(calc(100vh - ${navbarHeight}px), 775px)`,
        mt: `${navbarHeight}px`,
        pt: { xxs: 0 },
        display: "grid",
        gap: 8,
    ...
    >
        ...
        {images.map((image1, index) => (
          <Box key={index} item xxs={1} sx={itemStyles(index)}>
            {backgroundImages.map((img, idx) => (
              <Box key={idx} sx={imageStyles(backgroundImages[0])} />
            ))}
          </Box>
        ))}
      </Box>
      <Container
        ...
        sx={{
          ...
          alignItems: { xxs: "center", mobile: "flex-end" },
          height: 1,
          gap: "inherit",
        }}
      >
        <Box
          ...
          sx={{
            width: 1,
            height: 1,
            maxWidth: { xxs: 1, mobile: "xs", sm: "mobile" },
        ...
        <Box
          ...
          sx={{
            width: 1,
            maxWidth: { xxs: 300, md: 425 },
            justifySelf: { xxs: "flex-start", lg: "center" },
          }}
        >
          <ArrowLink sx={{ width: 1 }}>Request A Free Consulation</ArrowLink>
        </Box>
      </Container>
    </Paper>
  );

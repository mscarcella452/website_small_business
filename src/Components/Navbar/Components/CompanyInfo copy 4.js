...
function CompanyInfo({ handleCloseMenu }) {
  ...
  useEffect(() => {
    // Update the height state with the target container's height

    if (backButtonRef.current && containerRef.current) {
      containerRef.current.style.marginBottom = `${backButtonRef.current.clientHeight}px`;
    }
  }, []);

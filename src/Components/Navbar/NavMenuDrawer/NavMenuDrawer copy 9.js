  ...
  return (
    <>
      ...
      <NavMenuContext.Provider value={{ transitionState: transition }}>
        <Box
          className={classes.navMenu_container[screenSizeThreshold]}
        ...
        >
          {screenSizeThreshold && (
            <TransitionBox variant='horizontal' justifySelf={"flex-start"}>
              ...
              >
                <Typography
                  variant={screenSizeThreshold ? "h5" : "p"}
              ...
              <ContentBox
                className='background_image_navMenu'
                left={0}
                width={"60vw"}
                sx={{
          ...
          >
            ...
            <ContentBox
              className={
                !screenSizeThreshold &&
                (landscapeSm ? "fullScreen landscape" : "fullScreen vertical")
              }
              variant='transition'
              bottom={0}
              height={
                !screenSizeThreshold ? `calc(100vh - ${navbarHeight})` : "100vh"
              }
              sx={{ top: !screenSizeThreshold ? navbarHeight : 0 }}
            >
              <Box
...
function ContactInfo() {
  const theme = useTheme();
  const screenSizeThreshold = useMediaQuery(theme.navbar.screenSizeThreshold);
  const notSmall = useMediaQuery(xs);
  const fontSize = !notSmall && 10;
  return (
    <Box
      className='flexColumn'
      sx={{
...
function CompanyHours() {
  ...
  const day = screenSizeThreshold ? "day" : "abrievDay";
  return (
    <Box
      className='flexColumn'
      sx={{
      ...
      {links.map((link, index) => (
        <>
          <Link
            ...
            key={index}
            onClick={onClick}
            variant={screenSizeThreshold ? "h2" : "h5"}
            // href={link.to}
            to={link.to}
            typography='playfair'
            component={NavLink}
